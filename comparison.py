...
def calculate_purity(state_node_mappings, total_visits):
    """Calculate purity metric based on the mathematical formula"""
    if total_visits == 0:
        return 0
...
def calculate_se(transitions, agent, tau=0.0001):
    """
    Calculate sensorimotor representation error (SE) with consistent criteria
    ...
    for prev_state, curr_state in transitions:
        ...
        if isinstance(agent, TMGWRAgent):
            # TMGWR calculation remains unchanged
            ...
            # For MINERVA - use only connection-based check
            ...
            if prev_node_idx is not None and curr_node_idx is not None:
                if prev_node_idx < len(agent.connections) and curr_node_idx < len(agent.connections):
    ...
    for noise_level in noise_levels:
        ...
        for episode in range(episodes_per_noise):
            ...
            try:
                ...
                hgwrsom_agent.higher_nodes = []
                hgwrsom_agent.higher_connections = np.zeros((0, 0))
                ...
                for agent, agent_name in [(tmgwr_agent, 'TMGWR'), (hgwrsom_agent, 'MINERVA')]:
                    ...
                    while current_state != goal and step_counter < 20000:
                        ...
                        if agent_name == 'TMGWR':
                            ...
                            found = False
                            node_idx = 0
                            for idx, existing_pattern in enumerate(agent.higher_nodes):
                            ...
                            if not found:
                                ...
                                node_idx = len(agent.higher_nodes)
                                agent.higher_nodes.append(pattern)
                                
                                # Expand connections matrix
                                old_size = len(agent.higher_connections)
                                new_connections = np.zeros((old_size + 1, old_size + 1))
                                if old_size > 0:  # Only copy if there were existing connections
                                    new_connections[:-1, :-1] = agent.higher_connections
                                agent.higher_connections = new_connections
                    ...
                    if agent_name == 'TMGWR':
                        ...
                        num_nodes = len(agent.higher_nodes)
